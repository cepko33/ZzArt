...
export function UpdateFeedbackUI() {
  ...
  if (fs.useFeedback && state.feedbackClearOnChange) ClearFeedback();

  if (state.showPreview && fs.IsVariation()) fs.Render(true);
...
export function highlightShader(code) {
  ...
  keywords.forEach((word) => {
    ...
    highlighted = highlighted.replace(
      re,
      `<span class="glsl-keyword">${word}</span>`,
    );
  });
  ...
  builtins.forEach((word) => {
    ...
    highlighted = highlighted.replace(
      re,
      `<span class="glsl-builtin">${word}</span>`,
    );
  });
...
export function UpdateUI() {
  ...
  if (shader) document.title = `ZzArt - ` + shader.GetGenerationString();
  ...
  if (state.canvas_main) {
    ...
    let wa = w / h;
    if (state.rotateCanvas || true) wa = 1 / wa;
    if (wa > a) w = h * a;
    else h = w / a;
...
export function ChangeMemoryLocation(direction) {
  if (
    state.shaderMemoryLocation + direction < 0 ||
    state.shaderMemoryLocation + direction >= state.shaderMemory.length
  )
    return;
...
export function ButtonSeed() {
  let seedString = window.prompt(
    "Enter a ZzArt seed to use for randomization:",
    "",
  );
  ...
  if (!Number.isInteger(seed)) seed = HashString(seedString);
  if (seed == 0) seed = 1;
...
export function ButtonToggleFeedback() {
  ...
  if (
    state.favoriteShader.useFeedback &&
    !state.showPreview &&
    state.favoriteShader.IsVariation()
  )
    ButtonTogglePreview();
  else UpdateUI();
}
...
export function ButtonTogglePreview() {
  ...
  if (state.showPreview) {
    ...
    if (state.favoriteShader && state.favoriteShader.IsVariation()) {
      ClearFeedback();
      for (let i = 0; i < 10; i++) state.favoriteShader.Render(true);
    }
  }
...
export function ButtonSave() {
  ...
  let filename =
    "ZzArt-" + state.favoriteShader.GetGenerationString(1) + ".png";
...
export function SaveCode() {
  let filename =
    "ZzArt-" + state.favoriteShader.GetGenerationString(1) + ".txt";
  download(
    state.favoriteShader.GetCode(),
    filename,
...
export function RandomizeShaders(keepFeedback = false) {
  ...
  if (keepFeedback) {
    ...
    if (oldShader) {
      ...
      state.favoriteShader.chromaKeyColor = oldShader.chromaKeyColor.Clone
        ? oldShader.chromaKeyColor.Clone()
        : Object.assign(new Vector3(), oldShader.chromaKeyColor);
  ...
  for (let i = 0; i < 2; i++) {
    if (state.gridRenderQueue.length > 0) {
      ...
      if (shader.useFeedback) {
        ClearFeedback();
        for (let j = 0; j < 10; j++) shader.Render(true);
      } else {
        shader.Render();
      }

      const record = state.gridRenderRecords.find(
        (r) => r.x === x && r.y === y,
      );
      if (record) {
...
export function SetBest(bestX, bestY) {
  if (state.feedbackClearOnChange) ClearFeedback();
  ...
  if (state.feedbackLocked && state.favoriteShader) {
    ...
    target.chromaKeyColor = source.chromaKeyColor.Clone
      ? source.chromaKeyColor.Clone()
      : Object.assign(new Vector3(), source.chromaKeyColor);
  ...
  if (state.favoriteShader.generation == 0) ++state.favoriteShader.generation;
...
export function MakeVariations(skipX, skipY) {
  ...
  for (let X = 0; X < state.gridSize; X++)
    for (let Y = 0; Y < state.gridSize; Y++) {
      ...
      if (X != skipX || Y != skipY) shader.Mutate();
    }
}
...
export function SetFavoriteFromMemory() {
  ...
  if (state.favoriteShader.IsVariation()) {
    if (
      state.favoriteShader.gridPosX >= state.gridSize ||
      state.favoriteShader.gridPosY >= state.gridSize
    )
...
export function DisplaySaveListPage(saveMemory = 1) {
  ...
  for (let Y = 0; Y < state.gridSize; Y++)
    for (let X = 0; X < state.gridSize; X++) {
      ...
      if (i < state.saveList.length) shader = state.saveList[i].Clone();
      state.shaderGrid[X][Y] = shader;
      shader.SetGridPos(X, Y);
    }

  if (state.saveListIndex >= state.saveList.length) state.saveListIndex = 0;
...
export function UpdateSatelliteMode() {
  ...
  if (localStorageItem) {
    ...
    if (saveData.lastUpdate != state.lastSatelliteUpdate) {
      ...
      const feedbackToggledOn =
        !oldShader?.useFeedback && newShaderRaw.useFeedback;

      if (identityChanged || feedbackToggledOn) ClearFeedback();

      state.lastSatelliteUpdate = saveData.lastUpdate;
      if (saveData.simulationSpeed !== undefined) {
        state.settings.simulationSpeed = saveData.simulationSpeed;
      }
      state.favoriteShader = Object.assign(
        new ShaderObject(),
        rawObject,
      ).Clone();
    }
...
export function InitSatelliteMode() {
  if (!state.favoriteShader) state.favoriteShader = new ShaderObject();
...
export function TryToRotate() {
  ...
  if (state.rotateCanvas == newRotateCanvas) return;
