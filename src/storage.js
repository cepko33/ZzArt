...
export function LoadLocalStorage() {
    if (localStorage.version != state.config.dataVersion) return;
...
export function LoadSettingsFromCookie() {
    ...
    try {
        ...
        if (settings.showWatermark !== undefined)
            state.settings.showWatermark = settings.showWatermark;
...
export function SelectSavedShader(i) {
    i = parseInt(i);
    if (i >= state.saveList.length) return;

    if (state.saveList[i].IsSaveList()) return;
...
export function ExportSaveList() {
    ...
    let output = '';
    for (let shader of state.saveList) output += JSON.stringify(shader) + '\n';
