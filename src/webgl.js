...
export function RenderShader(code, feedbackOpts) {
    ...
    if (feedback) {
        ...
        const rawIdx = 0;
        const prevIdx =
            state.feedback.index === 1 || state.feedback.index === 2 ? state.feedback.index : 1;
        ...
        x.uniform1f(
            x.getUniformLocation(compProg, 'iFeedbackModAmount'),
            feedbackOpts.feedbackModAmount ?? 0
        );
        ...
        x.uniform1f(
            x.getUniformLocation(compProg, 'iChromaThreshold'),
            feedbackOpts.chromaThreshold ?? 0.1
        );
        x.uniform1f(
            x.getUniformLocation(compProg, 'iChromaSoftness'),
