...
export class ShaderObject {
  ...
  Clone() {
    ...
    for (let statement of this.shaderStatements)
      clone.shaderStatements.push(
        Object.assign(new ShaderStatement(), statement),
      );
    clone.paletteColors = [];
    ...
    clone.iterationCount = Clamp(
      parseInt(clone.iterationCount),
      1,
      state.maxIterations,
    );
    return clone;
  ...
  Mutate() {
    ...
    if (Rand() < 0.1) {
      ...
      this.uvOffsetX += RandBetween(-0.1, 0.1);
      this.uvOffsetY += RandBetween(-0.1, 0.1);
      this.uvScaleX += RandBetween(-0.1, 0.1);
      this.uvScaleY += RandBetween(-0.1, 0.1);
    }
    ...
    if (!state.feedbackLocked) {
      ...
      if (this.useFeedback) {
        ...
        if (Rand() < 0.08)
          this.feedbackBlur = Clamp(
            this.feedbackBlur + RandBetween(-0.1, 0.1),
            0,
            1,
          );
        if (Rand() < 0.1) {
  ...
  GetGenerationString(shorten) {
    ...
    if (this.IsSaveList()) {
      let page =
        (1 + this.saveListIndex / (state.gridSize * state.gridSize)) | 0;
