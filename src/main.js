...
import { InitSatelliteMode, SetFavoriteFromMemory, DrawShaders, UpdateUI, TryToRotate } from './ui/ui';
...
async function InitApp() {
    ...
    window.onkeydown = (e) => {
        ...
        else if (e.keyCode == 70) import('./ui/ui').then(m => m.ButtonToggleFeedback()), used = 1;
        else if (e.keyCode == 83) import('./ui/ui').then(m => m.ButtonSave()), used = 1;
        else if (e.keyCode == 90) import('./ui/ui').then(m => m.ChangeMemoryLocation(-1)), used = 1;
        else if (e.keyCode == 88) import('./ui/ui').then(m => m.ChangeMemoryLocation(1)), used = 1;
